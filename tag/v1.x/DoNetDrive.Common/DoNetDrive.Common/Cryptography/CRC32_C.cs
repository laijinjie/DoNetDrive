    ...
    public class CRC32_C
    {
        ...
        private static void Ini()
        {
            if (!IniCRCTable)
            {
                ...
                for (var i = 0; i <= 255; i++)
                    CRCTable[i] = Convert.ToUInt32(CRCTable_HEX[i], 16);
        ...
        public static UInt32 CalculateDigest(byte[] data, UInt32 offset, UInt32 size)
        {
            Ini();
            // Dim iMod = size Mod 4
            // If iMod > 0 Then
            ...
            UInt32 iDigest = UInt32.MaxValue;
            UInt32 order = 32;
            var iEnd = offset + size - 1;
            for (uint i = offset; i <= iEnd; i++)
                iDigest = (iDigest << 8) ^ CRCTable[((iDigest >> (int)(order - 8)) & 255) ^ data[i]];

            // iDigest = iDigest Xor UInt32.MinValue
            // iDigest = iDigest And UInt32.MaxValue
