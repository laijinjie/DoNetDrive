Public Class MyCRC32
    Private Shared CRCTable_HEX() As String = {"00000000", "04C11DB7", "09823B6E", "0D4326D9", "130476DC", "17C56B6B",
    ...
    Private Shared CRCTable As UInt32()
    Private Shared IniCRCTable As Boolean
    Private Shared Sub Ini()
        If Not IniCRCTable Then
    ...
    Public Shared Function CalculateDigest(data As Byte(), offset As UInt32, size As UInt32) As UInt32
        Ini()

        Dim iDigest As UInt32 = UInt32.MaxValue
        Dim order As UInt32 = 32
        Dim iEnd = offset + size - 1
        For i As UInteger = offset To iEnd
            iDigest = (iDigest << 8) Xor CRCTable(((iDigest >> CInt(order - 8)) And 255) Xor data(i))
        Next
    ...
    ''' <param name="offset">偏移，一般设为0。</param>
    ''' <param name="size">大小，一般设为数据组的Length。</param>
    Public Shared Function CalculateDigest_Continue(iDigest As UInt32, data As Byte(), offset As UInt32, size As UInt32) As UInt32
        Dim order As UInt32 = 32
        Dim iEnd = offset + size - 1
        For i As UInteger = offset To iEnd
            iDigest = (iDigest << 8) Xor CRCTable(((iDigest >> CInt(order - 8)) And 255) Xor data(i))
